...
(function(){
    ...
    if(!vid)return;
    function tryPlay(){vid.play().catch(()=>{})}
    tryPlay();
    // Also try on first user interaction (helps iOS)
    ['touchstart','touchend','scroll','click'].forEach(evt=>{
        document.addEventListener(evt,function h(){tryPlay();document.removeEventListener(evt,h)},{passive:true,once:true});
    });
    // Retry every 500ms for first 5s (covers slow network loads)
    let attempts=0;
    const retry=setInterval(()=>{if(vid.paused&&attempts<10){tryPlay();attempts++}else clearInterval(retry)},500);
})();
...
function checkout(product){
    openWaitlist(product || 'Insignia T-Shirt');
}
...
(function(){
  ...
  cards.forEach(card=>{
    ...
    const d0=card.querySelector('[data-dot="0"]');
    const d1=card.querySelector('[data-dot="1"]');
    if(!d0||!d1)return;
    function setGallery(idx){
      imgs[0].style.opacity=idx===0?'0.88':'0';
      imgs[1].style.opacity=idx===1?'0.88':'0';
      d0.style.background=idx===0?'rgba(154,127,74,0.6)':'rgba(234,234,234,0.2)';
      d1.style.background=idx===1?'rgba(154,127,74,0.6)':'rgba(234,234,234,0.2)';
    }
    let current=0;
    let startX=0;
